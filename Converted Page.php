 ...
 * Colorado Lottery — CO4 / CO5
 * Joomla 5.x + PHP 8.1+
 *
 * PURPOSE
 * - Premium, agency-grade results and analysis experience
 * - Calm, analytical presentation aligned with LottoExpert brand standards
 * - WCAG-aware contrast and structured heading hierarchy
 * - Joomla-safe APIs and properly escaped output throughout
 *
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $leMetaDesc = 'Frequency analysis, recency trends, and hot and cold numbers for ' . $stateName . ' ' . $gName . '. See the latest draw results and historical frequency data on LottoExpert.net.';
    $doc->setMetaData('description', htmlspecialchars($leMetaDesc, ENT_QUOTES, 'UTF-8'));
}
...
$leJsonLd = [
    ...
    '@type'       => 'WebPage',
    'name'        => isset($stateName, $gName) ? htmlspecialchars($stateName . ' ' . $gName . ' Results Intelligence', ENT_QUOTES, 'UTF-8') : 'Lottery Results Intelligence',
    'description' => isset($leMetaDesc) ? htmlspecialchars($leMetaDesc, ENT_QUOTES, 'UTF-8') : '',
...
$doc->addCustomTag('<script type="application/ld+json">' . json_encode($leJsonLd, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . '</script>');
?>
<style>
:root{
  --skai-blue:#1C66FF;
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    ...
    <div class="skai-section-body">
      ...
      <div class="skai-utility-grid">
        ...
        <a class="skai-utility-link" href="/lottery-archives-pick6?gId=<?php echo rawurlencode($gameId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="/lowest-drawn-number-analysis?gId=<?php echo rawurlencode($gameId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
...
(function () {
  ...
  function renderCharts() {
    ...
    if (topActiveCanvas && !topActiveCanvas._skaiChart) {
      topActiveCanvas._skaiChart = new Chart(topActiveCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (quietCanvas && !quietCanvas._skaiChart) {
      quietCanvas._skaiChart = new Chart(quietCanvas.getContext('2d'), {
        type: 'bar',
        data: {
    ...
    if (fullMainCanvas && !fullMainCanvas._skaiChart) {
      fullMainCanvas._skaiChart = new Chart(fullMainCanvas.getContext('2d'), {
        type: 'bar',
        data: {
